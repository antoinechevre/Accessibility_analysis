"""
Génère un nuage de points HTML interactif (autonome, sans serveur, Plotly.js
via CDN) à partir de output/index_benchmark_reseaux.csv — cf. la cellule
"#sauvegarde index" du notebook et le bouton "Enregistrer les indicateurs de
ce run" de l'onglet Accessibilité, qui alimentent ce fichier.
...
la page générée, sans avoir à relancer ce script :
...
def generer_html(df, chemin_sortie):
    ...
    for col in ("reseau", "ville_principale", "domaine", "nom_domaine", "decile"):
        if col not in df.columns:
            raise ValueError(f"Colonne attendue absente de {chemin_sortie!s} : {col}")
    ...
    domaines_dispo = (
        df[["domaine", "nom_domaine"]].drop_duplicates().sort_values("domaine").values.tolist()
    )
...
function redessiner() {
  ...
  const traces = deciles.map(decile => {
    const pts = filtre.filter(l => l.decile === decile);
    return {
      ...
      marker: { size: 10, color: couleurDecile(decile), line: { width: 2, color: couleurAnneau } },
      hovertemplate:
