...
- **Fond de carte CartoDB (Positron/Dark Matter)** : CARTO a coupé l'accès anonyme à ses tuiles `basemaps.cartocdn.com` (tuiles servies filigranées "API KEY REQUIRED" sans clé). Secret optionnel `CARTO_API_KEY` (Settings → Variables and secrets), obtenu gratuitement sur [carto.com/basemaps/apikey](https://carto.com/basemaps/apikey/) — sans lui (`fond_carte_kwargs`, `src/cartographie.py`), l'app retombe automatiquement sur OpenStreetMap.
